...
.cellBarcode_expect <- function( bccount, bcfile, outfilename=NULL) {
  ...
  if(nchar(bccount[keep==TRUE,XC][1]) != nchar(bc_wl[1])){
    ...
    bc_definition <- sapply(sequence_files, function(x) grep("BC", x$base_definition, value = T))
    ...
    if(length(bc_definition)>1){ #this only makes sense if there are at least 2 BC pieces defined
      ...
      bc_matched <- parallel::mcmapply(function(x) grep(pattern = x, x =search_vector), bc_wl, mc.cores = num_threads, mc.preschedule = TRUE)
...
cellBC<-function(bcfile, bcnum, bcauto, bccount_file, outfilename=NULL){
  ...
  }else if( is.null(bcfile)==FALSE & bcauto==FALSE ){
...
BCbin <- function(bccount_file, bc_detected) {
  ...
  if(barcode_binning>0){
    ...
    for(i in 1:length(true_chunks)){
      dists <- stringdist::stringdistmatrix(true_chunks[[i]],nocell_BCs,method="hamming", nthread = num_threads)
  ...
  if(!is.null(barcode_sharing)){
    share_table <- data.table::fread( barcode_sharing, header = F, skip = 1, fill = TRUE)
    ...
    share_mode <- data.table::fread( barcode_sharing, header = F, nrows = 1)$V1
