#!/usr/bin/env Rscript
options(warn = -1)
# Read in parameters from command line arguments
suppressMessages(require(R.utils))
opt<-commandArgs(trailingOnly = T,asValues=T)
# read input file
input<-read.csv(opt$input)
#read in file containing read pair identified read pair identification process
fq_pair <- as.list(readLines(file(opt$pairs_file)))
# filter input csv down to read pair being processed
input <- input[grep((paste(fq_pair,collapse="|")),input$name),]
# Convert input into yaml style list object
...
#process BC/UMI filtering options
BC_filter <- NULL
...
# Print options to output for perl script
